...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f z@(ListZipper l a r)
  | f a = IsZ z
  | otherwise = case l of
    (x :. xs) -> findLeft f (ListZipper xs x (a :. r))
    _ -> IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f z@(ListZipper l a r)
  | f a = IsZ z
  | otherwise = case r of
    (x :. xs) -> findRight f (ListZipper (a :. l) x xs)
    _ -> IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z@(ListZipper _ _ r) =
  let mz' = moveRightN (length r) z
  in case moveLeft z of
    IsZ z'  -> z'
    IsNotZ  -> case mz' of
      IsZ z'  -> z'
      IsNotZ  -> z  -- can't happen
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z@(ListZipper l _ _) =
  let mz' = moveLeftN (length l) z
  in case moveRight z of
    IsZ z'  -> z'
    IsNotZ  -> case mz' of
      IsZ z'  -> z'
      IsNotZ  -> z  -- can't happen
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n z@(ListZipper l _ r) = case moveRightN n z of
  IsZ z' -> Right z'
...
instance Apply ListZipper where
  ListZipper fl fa fr <*> ListZipper al aa ar =
    ListZipper (zipWith id fl al) (fa aa) (zipWith id fr ar)
...
instance Extend ListZipper where
  f <<= z = ListZipper
    (f <$> unfoldr (((\z' -> (z', z')) <$>) . toOptional . moveLeft) z)
    (f z)
    (f <$> unfoldr (((\z' -> (z', z')) <$>) . toOptional . moveRight) z)
...
instance Traversable ListZipper where
  traverse f (ListZipper l a r) =
    ListZipper <$> traverse f l <*> f a <*> traverse f r
